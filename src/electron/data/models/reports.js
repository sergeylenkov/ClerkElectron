...
export function getExpensesByMonth(from, to) {
    return new Promise((resolve, reject) => {
        db.all(`SELECT t.paid_at AS date, TOTAL(t.from_account_amount) AS sum FROM transactions t, accounts a
                 ...
                 GROUP BY strftime('%Y %m', t.paid_at) ORDER BY date`, [from.toISOString(), to.toISOString()], (error, rows) => {
            if (error) {
                reject(error);
            } else {
                let items = [];
                ...
                rows.forEach((row) => {
                    ...
                    const promise = getExpensesForMonth(date.format('YYYY MM')).then((expenses) => {
...
export function getExpensesForMonth(date) {
    return new Promise((resolve, reject) => {
